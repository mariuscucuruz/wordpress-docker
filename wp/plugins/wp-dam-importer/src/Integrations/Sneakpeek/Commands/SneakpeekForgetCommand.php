...
class SneakpeekForgetCommand extends Command
{
    use Loggable;

    public const string SIGNATURE = 'media:forget:sneeakpeek';
    ...
    protected $description = 'Rerun Sneakpeek.';
        ...
        if ($fileId) {
            ...
            $files = File::query()
                ...
                ->whereHas('operationStates', fn ($q) => $q
                    ...
                    ->when(! $force, fn ($q2) => $q2->where('status', FileOperationStatus::FAILED))
                )
                ->cursor();
