...
class NataeroSneakpeekDispatchJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        if (! $nataero->processOperation($this->file, new OperationParams(nataeroFunctionType: NataeroFunctionType::SNEAKPEEK))) {
            $this->log("Sneakpeek failed for File ID {$this->file->id}", 'error');

            $this->fail(new RuntimeException('Nataero Sneakpeek failed'));
        }
    }
    ...
    {
        $this->log(
            text: 'Nataero sneakpeek dispatch job failed',
            level: 'error',
            icon: '❌',
            context: [
