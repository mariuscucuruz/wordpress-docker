...
class NataeroExifDispatchJob implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        if (! $nataero->processOperation($this->file, new OperationParams(nataeroFunctionType: NataeroFunctionType::EXIF))) {
            $this->log("MediaExif failed for File ID {$this->file->id}", 'error');

            $this->fail(new RuntimeException('Nataero exif failed'));
        }
    }
    ...
    {
        $this->log(
            text: 'Nataero exif dispatch job failed',
            level: 'error',
            icon: '❌',
            context: [
