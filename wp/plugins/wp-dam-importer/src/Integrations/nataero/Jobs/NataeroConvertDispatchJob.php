...
class NataeroConvertDispatchJob implements ShouldBeUnique, ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Loggable, Queueable, SerializesModels;

    public function __construct(public File $file, public ?string $conversionType = null)
    {
    ...
    {
        if (! $nataero->processOperation($this->file, new OperationParams(nataeroFunctionType: NataeroFunctionType::CONVERT, type: $this->conversionType))) {
            $this->log("Conversion failed for File ID {$this->file->id}", 'error');

            $this->fail(new \RuntimeException('Nataero conversion failed'));
        }
    }
    ...
    {
        $this->log(
            text: 'Nataero convert dispatch job failed',
            level: 'error',
            icon: '❌',
            context: [
